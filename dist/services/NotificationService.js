...
class NotificationService {
    broadcastToMatch(match, message, excludePlayerId) {
    ...
    personalizeMessage(message, playerId) {
        ...
        if (personalized.type === 'player_joined') {
            ...
            if (payload.players?.turnOrder && Array.isArray(payload.players.turnOrder)) {
                payload.players.turnOrder = payload.players.turnOrder.map((p) => ({
                    name: p.name,
                    teamId: p.teamId,
            ...
            if (payload.teams) {
                payload.teams = {
                    team1: Array.isArray(payload.teams.team1) ? payload.teams.team1.map((p) => ({
                        name: p.name,
                        id: p.name === "you" ? playerId : undefined
                    })) : [],
                    team2: Array.isArray(payload.teams.team2) ? payload.teams.team2.map((p) => ({
                        name: p.name,
                        id: p.name === "you" ? playerId : undefined
                    })) : []
                };
            ...
            if (payload.playedCards && Array.isArray(payload.playedCards)) {
                payload.playedCards = payload.playedCards.map((card) => ({
