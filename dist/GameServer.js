...
const RoundEvaluator_1 = require("./lib/RoundEvaluator");
...
class GameServer {
    constructor(port = 8080) {
        ...
        this.activePlayers = new Set(); // Track active playerIds across all matches
        this.roundEvaluators = new Map(); // Track evaluator per match
        this.httpServer = (0, http_1.createServer)(this.handleHttpRequest.bind(this));
        this.wss = new ws_1.Server({ server: this.httpServer });
        this.deckService = new DeckService_1.DeckService();
        ...
        this.wss.on('connection', (ws, req) => {
            ...
            ws.on('message', (data) => {
                try {
                    ...
                    const errorMessage = types_1.MessageBuilder.createError('INVALID_MESSAGE', 'Invalid message format');
    ...
    handlePlayerConnection(ws, match, teamId, providedPlayerId, providedName) {
        ...
        const welcomeMessage = types_1.MessageBuilder.createConnectionEstablished(playerInfo, matchSummary, {
        ...
        const playerJoinedMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.PLAYER_JOINED, playerJoinedPayload);
    ...
    handlePlayerReconnection(ws, match, existingPlayer) {
        ...
        const reconnectMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.RECONNECTION_SUCCESSFUL, reconnectPayload);
        ...
        const playerReconnectedMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.PLAYER_RECONNECTED, playerReconnectedPayload);
        ...
        if (match.status === 'paused' && this.allPlayersConnected(match)) {
            ...
            Array.from(match.players.values()).forEach(p => {
                if (p.ws && p.connected) {
                    ...
                    const resumeMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.MATCH_RESUMED, resumePayload);
    ...
    handleMessage(ws, message) {
        ...
        switch (type) {
            case types_1.MessageType.PLAY_CARD_REQUEST:
                this.handlePlayCard(ws, payload);
                break;
            case types_1.MessageType.GET_STATE_REQUEST:
                this.handleGetGameState(ws);
                break;
            default:
                const errorMessage = types_1.MessageBuilder.createError('UNKNOWN_MESSAGE_TYPE', 'Unknown message type');
                ws.send(JSON.stringify(errorMessage));
        }
    }
    handleGetGameState(ws) {
        ...
        if (!matchId || !playerId) {
            const errorMessage = types_1.MessageBuilder.createError('INVALID_STATE', 'Invalid connection state');
            ws.send(JSON.stringify(errorMessage));
            return;
        }
        const match = this.matches.get(matchId);
        if (!match) {
            const errorMessage = types_1.MessageBuilder.createError('MATCH_NOT_FOUND', 'Match not found');
            ws.send(JSON.stringify(errorMessage));
            return;
        }
        const player = match.players.get(playerId);
        if (!player) {
            const errorMessage = types_1.MessageBuilder.createError('PLAYER_NOT_FOUND', 'Player not in match');
            ws.send(JSON.stringify(errorMessage));
            return;
        }
        // Send current game state
        const gameState = this.createGameState(match, playerId);
        const gameStateMessage = types_1.MessageBuilder.createGameStateUpdate(gameState);
    ...
    handlePlayCard(ws, payload) {
        ...
        if (!matchId || !playerId) {
            const error = types_1.MessageBuilder.createError('INVALID_STATE', 'Invalid connection state');
            ws.send(JSON.stringify(error));
            return;
        }
        const match = this.matches.get(matchId);
        if (!match) {
            const error = types_1.MessageBuilder.createError('MATCH_NOT_FOUND', 'Match not found');
            ws.send(JSON.stringify(error));
            return;
        }
        const player = match.players.get(playerId);
        if (!player) {
            const error = types_1.MessageBuilder.createError('PLAYER_NOT_FOUND', 'Player not in match');
            ws.send(JSON.stringify(error));
            return;
        }
        if (match.status !== 'active') {
            const error = types_1.MessageBuilder.createError('MATCH_NOT_ACTIVE', 'Match is not active');
            ws.send(JSON.stringify(error));
            return;
        }
        if (match.currentPlayerId !== playerId) {
            const error = types_1.MessageBuilder.createError('NOT_YOUR_TURN', 'It is not your turn to play');
            ws.send(JSON.stringify(error));
            return;
        }
        ...
        if (cardIndex === -1) {
            const error = types_1.MessageBuilder.createError('INVALID_CARD', 'Card not in your hand. Your hand on the server may be out of sync.', {
        ...
        Array.from(match.players.values()).forEach(p => {
            if (p.ws && p.connected) {
                ...
                const message = types_1.MessageBuilder.createMessage(types_1.MessageType.CARD_PLAYED, cardPlayedPayload);
    ...
    completeRound(match) {
        ...
        const moves = match.playground.map(playedCard => ({
        ...
        Array.from(match.players.values()).forEach(p => {
            if (p.ws && p.connected) {
                ...
                const message = types_1.MessageBuilder.createMessage(types_1.MessageType.ROUND_COMPLETED, roundCompletedPayload);
                p.ws.send(JSON.stringify(message));
            }
        });
    ...
    completeMatch(match) {
        ...
        const winnerTeamId = match.teamScores.team1 > match.teamScores.team2 ? 'team1' : 'team2';
        const stats = {
        ...
        Array.from(match.players.values()).forEach(p => {
            if (p.ws && p.connected) {
                const gameState = this.createGameState(match, p.id);
                const matchEndedMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.MATCH_ENDED, { gameState });
    ...
    notifyTurnChange(match) {
        ...
        Array.from(match.players.values()).forEach(p => {
            if (p.ws && p.connected) {
                ...
                const message = types_1.MessageBuilder.createMessage(types_1.MessageType.TURN_CHANGED, turnChangedPayload);
                p.ws.send(JSON.stringify(message));
            }
        });
    }
    startMatch(match) {
        ...
        this.roundEvaluators.set(match.id, new RoundEvaluator_1.RoundEvaluator(trumpSuit, 8, // totalRounds
        match.players.size));
        ...
        playersArray.forEach(player => {
            if (player.ws && player.connected) {
                ...
                const message = types_1.MessageBuilder.createMessage(types_1.MessageType.MATCH_STARTED, matchStartedPayload);
    ...
    handleDisconnection(ws) {
        ...
        if (match.status === 'active') {
            ...
            Array.from(match.players.values()).forEach(p => {
                if (p.ws && p.connected) {
                    ...
                    const message = types_1.MessageBuilder.createMessage(types_1.MessageType.MATCH_PAUSED, pausedPayload);
                    p.ws.send(JSON.stringify(message));
                }
            });
        }
        ...
        const disconnectedMessage = types_1.MessageBuilder.createMessage(types_1.MessageType.PLAYER_DISCONNECTED, disconnectedPayload);
    ...
    cleanupExpiredMatches(maxAgeHours = 24) {
        const now = Date.now();
    ...
    createPayloadPlayedCard(playedCard, match) {
    ...
    createGameState(match, forPlayerId) {
